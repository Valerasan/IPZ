...
namespace IPZ_1.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
    public class RegisterModel : PageModel
    {
        ...
        {
            if(!await _roleManager.RoleExistsAsync(WC.AdmineRole))
            {
                await _roleManager.CreateAsync(new IdentityRole(WC.AdmineRole));
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    if(User.IsInRole(WC.AdmineRole))
                    {
						await _userManager.AddToRoleAsync(user, WC.AdmineRole);
					}
                    else
                    {
